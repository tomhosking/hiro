...
for cat_slug, filter_cat in filter_cats.items():
    ...
    for split in ['valid','test']:
        ...
        entity_count = 0
        num_reviews = []
        file_list = os.listdir(f'/mnt/ext/phd/data/amasum/min_10_revs_filt_complete/{split}/')
        np.random.shuffle(file_list)

        raw_objs = []
        for file in tqdm(file_list):
            with open(f'/mnt/ext/phd/data/amasum/min_10_revs_filt_complete/{split}/' + file) as f:
                data = json.load(f)
